...
(function(exported) {
	...
	File.prototype.HDR_SPEED       = "\x00\x80";         // Defaults to 128 ticks per beat
	...
	File.prototype.setTicks = function(num) {
		var enc = '', hex;

		// Do nothing in case of invalid input
		if (!num || typeof num !== 'number') {
			return;
		}

		while (num) {
			hex = num % 256;             // Get the lowest two bytes
			num = Math.floor(num / 256); // Shift the number for the next iteration
			hex = hex.toString(16);      // Convert the two bytes to a string
			hex = ('0' + hex).slice(-2); // Pad, using slice.
			enc = '\\x' + hex + enc;     // Add special character, prepend to results.
		}
		this.HDR_SPEED = ('\\x00' + enc)
			// Handle the single byte case
			.slice(-8)
			// Convert to unicode (hack)
			.replace(/\\x([a-fA-F0-9]{2})/g, function(match, num) { 
				return JSON.parse('"\\u00'+num+'"'); 
			});
	}
