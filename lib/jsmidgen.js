...
(function(exported) {
	...
	Track.prototype.addNoteOn = Track.prototype.noteOn = function(channel, pitch, time, velocity) {
		this.events.push(new MidiEvent({
			...
			param2: velocity !== undefined && typeof velocity === 'number' ? velocity : DEFAULT_VOLUME,
			time: time || 0,
		}));
		return this;
	};
     ...
     * @param {number} denominator - The denominator of the time signature.
	...
	/**
	 * Set the key signature for the track.
	 *
     * @param {number} accidentals - The number of sharps (if positive) or
     * flats (if negative) in the key signature.
     * @param {boolean} [minor=false] - True if this is a minor key.
	 * @param {number} [time=0] - The number of ticks since the previous event,
	 * defaults to 0.
	 * @returns {Track} The current track.
	 */
	Track.prototype.setKeySignature = Track.prototype.keySignature = function(accidentals, minor, time) {
		this.events.push(new MetaEvent({
	...
	/**
	 * Serialize the MIDI file to a Uint8Array.
	 *
	 * @returns {Uint8Array} The array of serialized bytes.
	 */
    File.prototype.toUint8Array = function() {
        return Uint8Array.from(this.toBytes());
    };

	/**
	 * Serialize the MIDI file to a Blob.
	 *
     * @param {boolean} [genericType=false] - If false, uses the correct MIDI
     * MIME type. If true, uses the generic bytestream MIME type.
	 * @returns {Blob} The blob containing the serialized bytes.
	 */
    File.prototype.toBlob = function(genericType) {
